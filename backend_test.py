        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    # Verify report type
                    if report_data.get("report_type") != "monthly":
                        await self.log_test("Monthly Report Generation", False, f"Wrong report type: {report_data.get('report_type')}")
                        return False
                    
                    # Verify content contains expected sections
                    content = report_data.get("content", "")
                    expected_sections = ["Monthly Overview", "Applications by Status", "By Position Type", "By Work Mode", "Monthly Insights"]
                    missing_sections = [section for section in expected_sections if section not in content]
                    ...
                    await self.log_test("Monthly Report Generation", True, f"Generated report {report_data.get('report_id')} with all required sections")
                    return True
                else:
